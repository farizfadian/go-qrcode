...
| <img src="docs/images/qr-logo-letter.png" width="150"> | <img src="docs/images/qr-logo-target.png" width="150"> | <img src="docs/images/qr-logo-chevron.png" width="150"> |
|:---:|:---:|:---:|
| a letterform | a ring | a chevron |

Those marks are generated by [`testdata/samplelogos`](testdata/samplelogos),
not downloaded. Brand logos are registered trademarks, and putting one in an
unaffiliated repository invites trouble for no benefit.

#### A vector logo for SVG output

Go has no SVG rasteriser, so PNG, JPEG and WebP need a raster logo. But SVG
output does not have to rasterise anything — the artwork can be nested straight
into the document and rendered by whatever displays it:

```go
Logo: &qr.LogoOptions{
	Path:      "logo.png",          // used by PNG, JPEG and WebP
	SVGMarkup: string(svgBytes),    // used by SVG, embedded verbatim
}
```

`SVGMarkup` is an addition, not a replacement, and that is deliberate. An option
that let `New` succeed and `PNG` fail afterwards would break this library's rule
that configuration errors surface at `New` — and `Image` cannot return an error
at all, so it would have quietly handed back a code with no logo on it. Setting
